...
const ENGINEER_NAV: NavItem[] = [
  ...
  { to: "/engineer/review", label: "Technical Assurance Review", icon: FileSearch },
...
export function AppShell({
  ...
  return (
    <div className="min-h-screen bg-background lg:flex">
      <a
        href="#app-main"
        className="sr-only z-50 rounded-sm bg-primary px-3 py-2 text-sm font-medium text-primary-foreground focus:not-sr-only focus:fixed focus:left-3 focus:top-3"
      >
      ...
      <main id="app-main" className="flex min-h-screen min-w-0 flex-1 flex-col lg:min-h-screen">
        <header className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b bg-card/95 px-4 py-3 backdrop-blur lg:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <h1 className="truncate text-sm font-semibold">{current?.label ?? "Dashboard"}</h1>
            </div>
          </div>
          <div className="hidden shrink-0 rounded-sm border bg-muted/40 px-2 py-1 text-[10px] font-mono text-muted-foreground sm:block">
            Local-first | v0.1 MVP
          </div>
        </header>
        <div className="flex-1 overflow-y-auto p-3 sm:p-5 lg:p-6">{children}</div>
