  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border bg-card p-4">
        <div className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground mb-3">Progression Ladder</div>
        <div className="flex items-center gap-1">
          {COMPETENCY_LEVELS.map((l, i) => (
            <div key={l} className={`flex-1 px-2 py-2 text-center text-xs border ${i <= idx ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-muted-foreground'}`}>{l}</div>
          ))}
        </div>
      </div>
      <div className="border bg-card overflow-x-auto">
        <table className="w-full text-xs">
          <thead className="bg-muted/60 text-[10px] font-mono uppercase tracking-wider text-muted-foreground">
            <tr><th className="px-3 py-2 text-left">Scenario</th><th className="px-3 py-2 text-left">Category</th><th className="px-3 py-2 text-left">Score</th></tr>
          </thead>
          <tbody>
            {SCENARIOS.map((s) => (
              <tr key={s.id} className="border-t">
                ...
                <td className="px-3 py-1.5 font-mono">{results[s.id] !== undefined ? `${results[s.id]}%` : '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
